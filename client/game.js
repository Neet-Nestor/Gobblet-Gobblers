class GobbletGobblers {
    ...
    addPieceSelectionListeners() {
        ...
        buttons.forEach(button => {
            button.addEventListener('click', () => {
                ...
                document.getElementById('status').textContent = `Selected size: ${this.selectedSize}. Place your piece.`;
            });
        });
    }
    ...
    updateCell(row, col) {
        ...
        cellDiv.innerHTML = cell.map(piece => `<div class="${piece.size} ${piece.player}">${piece.size[0].toUpperCase()}</div>`).join('');
    }

    makeMove(row, col) {
        if (this.gameStarted && this.selectedSize && this.availablePieces[this.selectedSize] > 0) {
            this.ws.send(JSON.stringify({ type: 'move', row, col, size: this.selectedSize }));
        }
    }
    ...
    handleMessage(message) {
        ...
        switch (data.type) {
            ...
            case 'start':
                ...
                document.getElementById('status').textContent = `Game started! It's ${this.currentPlayer}'s turn.`;
                break;
            case 'move':
                this.board[data.row][data.col].push(data.piece);
                this.updateCell(data.row, data.col);
                this.updateAvailablePiecesDisplay();
                document.getElementById('status').textContent = `It's now ${data.currentPlayer}'s turn.`;
                break;
            case 'updatePieces':
            ...
            case 'updateTurn':
                this.currentPlayer = data.currentPlayer;
                document.getElementById('status').textContent = `It's now ${this.currentPlayer}'s turn.`;
                break;
            case 'playersCount':
                document.getElementById('players-count').textContent = `Connected Players: ${data.count}`;
                break;
        }
    }
}
